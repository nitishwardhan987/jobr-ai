export default function PrivacyPage() {
  ...
  return (
    <div style={{ background: '#F8F5F0', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(145deg, rgba(249,115,22,0.06) 0%, transparent 50%)', borderBottom: '1px solid #E7E5E4', padding: 'clamp(48px,6vw,80px) 20px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 100, padding: '5px 14px', marginBottom: 20 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#16A34A', fontFamily: 'monospace' }}>🔒 PRIVACY FIRST</span>
          </div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(32px,5vw,52px)', fontWeight: 900, color: '#18181B', letterSpacing: '-0.03em', marginBottom: 12 }}>Privacy Policy</h1>
          <p style={{ fontSize: 14, color: '#71717A' }}>Last updated: {lastUpdated}</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: 'clamp(32px,5vw,60px) 20px' }}>
        {[
          ...
          <div key={section.title} style={{ marginBottom: 36 }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, color: '#18181B', marginBottom: 12 }}>{section.title}</h2>
            ...
            {section.footer && <p style={{ fontSize: 14, color: '#71717A', marginTop: 12, padding: '10px 14px', background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: 8 }}>{section.footer}</p>}
          </div>
        ))}

        <div style={{ padding: '20px 24px', background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 16 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#16A34A', fontFamily: 'var(--font-display)', marginBottom: 6 }}>🔒 Privacy is not a feature. It's the foundation.</div>
          <p style={{ fontSize: 14, color: '#71717A', lineHeight: 1.65, margin: 0 }}>Jobr was architected with privacy as a core constraint, not an afterthought. Your CV data never hits our servers. Your API keys stay in your browser. That's a promise, not a policy.</p>
        </div>
      </div>
    </div>
  );
